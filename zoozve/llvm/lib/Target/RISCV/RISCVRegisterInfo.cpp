...
namespace opts {
llvm::cl::opt<unsigned> VenusNrRows(
    "venus-nr-row",
    llvm::cl::desc("Specify the number of rows of the Venus architecture."),
    llvm::cl::value_desc("value"),
    llvm::cl::init(2048)
);

llvm::cl::opt<unsigned> VenusNrLanes(
    "venus-nr-lane",
    llvm::cl::desc("Specify the number of lanes of the Venus architecture."),
    llvm::cl::value_desc("value"),
    llvm::cl::init(128)
);

unsigned VenusMaxRegisters = 1024;
} // namespace opts
...
BitVector RISCVRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  // For Venus Extensions, the registers should be handled according to the
  // command line specification, since the micro-architecture can be hetero-
  // genous.
  ...
  // instructions. The size of swap spaces is hardcoded as 128.
  for (unsigned I = RISCV::VNS0 + opts::VenusNrRows - 128;
       I != RISCV::VNS0 + opts::VenusNrRows; I++)
